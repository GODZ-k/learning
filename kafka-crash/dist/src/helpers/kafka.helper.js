import { admin, connectKafkaConsumer, consumer, producer, } from "../config/kafka.config.js";
export const produceMessage = async (topic, data) => {
    try {
...
const processMessages = (messages, isProcessing) => {
    ...
    try {
        ...
        await consumer.run({
            eachMessage: async ({ message, topic, heartbeat, partition, pause }) => {
                const parseMessage = message.value ? JSON.parse(message.value?.toString()) : "";
